...
namespace GoHijauBackend.Application.Services
{
    public class TransactionService
    {
        ...
        public async Task<CollectorCollectionStatsDto> GetCollectorCollectionStatsAsync(string userId)
        {
            var transactions = await _transactionRepository.GetTransactionsByCollectorId(userId);
            var timeZone = ResolveMalaysiaTimeZone();
            var nowLocal = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, timeZone);
            var todayStartLocal = nowLocal.Date;
            var daysFromMonday = ((int)nowLocal.DayOfWeek + 6) % 7;
            var weekStartLocal = todayStartLocal.AddDays(-daysFromMonday);

            double collectionsTodayKg = 0;
            double collectedThisWeekKg = 0;

            foreach (var transaction in transactions)
            {
                var kg = transaction.OilCollected > 0
                    ? transaction.OilCollected
                    : transaction.CollectorRecordedOil;
                if (kg <= 0)
                {
                    continue;
                }

                var createdUtc = DateTime.SpecifyKind(transaction.CreatedAt, DateTimeKind.Utc);
                var createdLocal = TimeZoneInfo.ConvertTimeFromUtc(createdUtc, timeZone);

                if (createdLocal.Date >= todayStartLocal)
                {
                    collectionsTodayKg += kg;
                }

                if (createdLocal.Date >= weekStartLocal)
                {
                    collectedThisWeekKg += kg;
                }
            }

            return new CollectorCollectionStatsDto
            {
                CollectionsTodayKg = Math.Round(collectionsTodayKg, 2),
                CollectionsThisWeekKg = Math.Round(collectedThisWeekKg, 2)
            };
        }

        private static TimeZoneInfo ResolveMalaysiaTimeZone()
        {
            foreach (var id in new[] { "Singapore Standard Time", "Asia/Kuala_Lumpur", "Asia/Singapore" })
            {
                try
                {
